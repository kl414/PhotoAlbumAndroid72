...
public class Control implements Controller{
	...
	public boolean rename(String oldName, String newName){
		...
		for(Album album: thisUser.albums){
			if(album.albumName.equalsIgnoreCase(oldName))
				target = album;
	...
	public String[][] listAlbums() {
		...
		Date minDate = null;
		Date maxDate = null;
		String[][] ret = new String[album.size()][];
		if(album == null || album.size() == 0){
			return null;
		}else{
			for (int i=0; i<album.size(); i++){
				photos = album.get(i).photos;
				//String tmpDate = df.format(album.get(i).photos.get(j).getCreationDateTime().getTime());
				Collections.sort(photos,new Comparator<Photo>()
						{
					@Override
					public int compare(Photo a, Photo b){
						return a.getCreationDateTime().before(b.getCreationDateTime()) ? -1 : a.getCreationDateTime().equals(b.getCreationDateTime()) ? 0 : 1;
					}
						});
				if (!photos.isEmpty()){
					minDate = photos.get(0).getCreationDateTime().getTime();
					maxDate = photos.get(0).getCreationDateTime().getTime();
					if (photos.get(photos.size()-1) != null){
						maxDate = photos.get(photos.size()-1).getCreationDateTime().getTime();
					}
					String tmpDate = df.format(minDate);
					String tmp2Date = df.format(maxDate);
					String[] temp = {album.get(i).albumName, album.get(i).photos.size()+"", tmpDate, tmp2Date};
					ret[i] = temp;
				}
				else{	
					String[] temp = {album.get(i).albumName, album.get(i).photos.size()+"", "", ""};
					ret[i] = temp;
				}
			}
		}
